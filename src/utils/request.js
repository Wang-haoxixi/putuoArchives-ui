...
service.interceptors.request.use(config => {
  ...
  //判断data是否需要加密
  if(config.data&&config.encryption=== 'ciphertext'){
  config.data = sm2.doEncrypt(JSON.stringify(config.data),publicKey)
  // console.error(sm2.doDecrypt(config.data,privateKey))
  }
  // get请求映射params参数
  ...
  if (config.method === 'get' && config.params) {
    ...
    //判断映射params参数是否需要加密
    if(config.encryption ==='ciphertext'){
      urlParams = sm2.doEncrypt(urlParams,publicKey)
    }
    config.params = {};
