...
export default {
  ...
  methods: {
    getStatistics () {
      getStatistics({library: this.$route.query.id}).then(({data}) => {
        const statistics = {}
    ...
    toAdd () {
      this.$router.push({
        path: "/keyword/dedicated/keyword/edit?library=" + this.library
      })
    },
    toEdit (row) {
      this.$router.push({
        path: `/keyword/dedicated/keyword/edit?library=${this.library}&id=${row.id}`
      })
    },
    toDelete (row) {
    ...
    handleImport () {
      selectFile().then(fileList => {
        importTemplate(this.library, fileList[0]).then(({data}) => {
